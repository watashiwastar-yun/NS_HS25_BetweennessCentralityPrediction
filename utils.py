...
def graph_to_adj(list_graph,list_n_sequence,list_node_num,model_size):
    ...
    for i in range(len(list_graph)):
        ...
        # get weighted adjacency matrix (assuming 'weight_AKA_average_time' stores distance/cost)
        # We need to specify the weight key explicitly.
        # If your graph edges have 'weight_AKA_average_time', use that key.
        adj_temp = nx.adjacency_matrix(graph, nodelist=node_sequence, weight='weight_AKA_average_time')
        
        # KEY STEP: Convert Distance/Cost to Affinity/Strength
        # Since smaller distance = stronger connection, we take the inverse.
        # We add a small epsilon to avoid division by zero.
        # Note: We only invert the existing edges (non-zero entries).
        ...
        # get degree
        # For the degree/mask calculation, we care about TOPOLOGY (existence of path).
        # So we temporarily create a binary version (1s and 0s) to count degrees and check cliques.
        binary_adj = adj_temp.copy()
        binary_adj.data[:] = 1.0
        
        arr_temp1 = np.sum(binary_adj, axis=1)
        arr_temp2 = np.sum(binary_adj.transpose(), axis=1)
